...
[Heaptrack](https://github.com/KDE/heaptrack) is a heap memory profiler for Linux, which _can_ also be instrumented to run on OSX.
...
Finally, from within XQuartz integrated terminal, run `xhost +localhost`. _IMPORTANT_ this has to be done each time XQuartz is restarted.
